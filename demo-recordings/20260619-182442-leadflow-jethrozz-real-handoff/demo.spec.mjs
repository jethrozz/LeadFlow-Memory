...
const runDir =
  ...
  "/Users/jethrozz/Documents/UGit/LeadFlow-Memory/demo-recordings/20260619-182442-leadflow-jethrozz-real-handoff";
...
async function installCaption(page) {
  await page.evaluate(() => {
    ...
    Object.assign(bar.style, {
      ...
      color: "#fff",
      textAlign: "center",
      font: '500 25px/1.35 system-ui, "PingFang SC", sans-serif',
      zIndex: "999999",
      opacity: "0",
    ...
    bar.innerHTML =
      '<div class="cn"></div><div class="en" style="font-size:16px;opacity:.78;margin-top:4px"></div>';
...
async function waitForNewTimelineEvent(baselineCount, type, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    const added = timeline.slice(baselineCount);
    if (added.length > 0 && (!type || added.some((event) => event.type === type))) {
      return detail;
    }
...
const baselineHandoffCount = initialDetail.timeline.filter((event) => event.type === "handoff_recovered").length;
...
try {
  ...
  const sentAfterStart = await waitForNewTimelineEvent(baselineTimelineCount, "agent_replied", 38_000);
  if (sentAfterStart) {
    log(`background worker produced timeline=${sentAfterStart.timeline.length}`);
  } else {
    log("background worker did not produce a new agent_replied event within the recording window");
  }
  await page.reload({ waitUntil: "domcontentloaded" });
  await waitForVisible(page.getByRole("heading", { name: "Conversion Agent Workbench" }));
  await waitForVisible(page.locator(".lead-row", { hasText: "jethrozz" }));
  await installCaption(page);
  ...
  await waitForVisible(page.getByText(/Handoff succeeded/), 130_000);
  ...
  if (recovered) {
    log(`handoff recovered timeline=${recovered.timeline.length}`);
  } else {
    const nowDetail = await leadDetail();
    const count = nowDetail.timeline.filter((event) => event.type === "handoff_recovered").length;
    log(`handoff recovery count baseline=${baselineHandoffCount} current=${count}`);
  }
  await page.reload({ waitUntil: "domcontentloaded" });
  await waitForVisible(page.getByRole("heading", { name: "Conversion Agent Workbench" }));
  await installCaption(page);
